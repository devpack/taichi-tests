...
# python3 nbody-dataclass.py --arch=cpu --body=8 --fps=-1
# python3 nbody-dataclass.py --arch=vulkan --body=8 --fps=60
...
class NBodySystem:
    ...
    def init(self):
        ...
        for i in range(self.nb_body):
            ...
            p.pos = [((ti.random(float) * 2) - 1) * 1.0, ((ti.random(float) * 2) - 1) * 1.0, ((ti.random(float) * 2) - 1) * 1.0]
            p.vel = [0.0, 0.0, 0.0]
            p.acc = [0.0, 0.0, 0.0]
            p.mass = 1.0
        ...
        for i in range(self.nb_body):
            for j in range(self.nb_body):
                if i != j:
                    ...
                    PHI = self.bodies[i].mass / (ti.sqrt(DR2) * DR2)
...
class App:
    ...
    def run(self):
        ...
        while self.window.running:
            ...
            self.scene.particles(self.nbody_system.bodies.pos, radius=0.01, color=(1.0, 1.0, 1.0))
